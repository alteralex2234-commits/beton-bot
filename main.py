...
async def run_fastapi(container: ServiceContainer) -> None:
    ...
    # Render требует bind на 0.0.0.0 и порт из переменной PORT
    host = "0.0.0.0"
    port = int(os.getenv("PORT", str(container.settings.app_port)))

    logger.info("FastAPI server starting on %s:%s", host, port)
